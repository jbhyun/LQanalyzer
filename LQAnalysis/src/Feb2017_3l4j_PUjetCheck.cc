...
//  TruthPrintOut();
   ...
   //if(false){
     ...
     //Edit the code to match without duplicate match
     //Check dR of jetjet 
     //Use only DY+jets since we only interest in impact of PU to jet selection
     //this is already done in 15' but I do this because we also need to check 16' since Nvtx diff.
     //Then check the plots with PU mva and how much it decreases.
     ...
     for(int i=0; i<jetColl.size(); i++){
       ...
       if(MatchedGenJetIdxTightColl.at(i) > 0){
        ...
        //Real Jet Basic Prop
         ...
         if((jetpt<30 && jetmva>-0.96) || (jetpt>=30 && jetmva>-0.92)){
         ...
         if((jetpt<30 && jetmva>-0.49) || (jetpt>=30 && jetmva>-0.06)){
         ...
         if((jetpt<30 && jetmva>0.26) || (jetpt>=30 && jetmva>0.62)){
