...
void AnalyzerCore::WriteHists(){
  ...
  for(map<TString, TH1*>::iterator mapit = maphist.begin(); mapit != maphist.end(); mapit++){
    ...
    else if(mapit->first.Contains("closejet")){
...
//If argument jetcoll is in Pt order, then by the algorithm returned bjetcoll will also be in Pt order
   ...
   for(unsigned int i=0; i<jetColl.size(); i++){
     if(level.Contains("Loose")){ if(jetColl.at(i).BJetTaggerValue(KJet::CSVv2) > 0.460) bjetColl.push_back(jetColl.at(i));}
     if(level.Contains("Medium")){ if(jetColl.at(i).BJetTaggerValue(KJet::CSVv2) > 0.800) bjetColl.push_back(jetColl.at(i));}
     if(level.Contains("Tight")){ if(jetColl.at(i).BJetTaggerValue(KJet::CSVv2) > 0.935) bjetColl.push_back(jetColl.at(i));}
   }
   return bjetColl;
}
...
std::vector<snu::KJet> AnalyzerCore::SelLightJets(std::vector<snu::KJet>& jetColl, TString level){
    ...
    for(vector<KJet>::iterator itj=jetColl.begin(); itj!=jetColl.end(); ++itj){
      if(level.Contains("Loose")){ if(itj->BJetTaggerValue(KJet::CSVv2)<0.460) ljetColl.push_back((*itj));}
      if(level.Contains("Medium")){ if(itj->BJetTaggerValue(KJet::CSVv2)<0.800) ljetColl.push_back((*itj));}
      if(level.Contains("Tight")){ if(itj->BJetTaggerValue(KJet::CSVv2)<0.935) ljetColl.push_back((*itj));}
    }
    return ljetColl;
}
