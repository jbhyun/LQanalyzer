...
float AnalyzerCore::MC_CR_Correction(TString analysis, int syst){
...
std::vector<snu::KJet> AnalyzerCore::GetJets(TString jetid, TString Option,float ptcut, float etacut){

  vector<snu::KMuon>  muons;
  vector<snu::KElectron>  electrons;

  return GetJets(jetid, muons, electrons, Option, ptcut,etacut);
}

std::vector<snu::KJet> AnalyzerCore::GetJets(TString jetid,  vector<snu::KMuon>  muons, vector<snu::KElectron>  electrons,TString Option,float ptcut, float etacut){
  
  std::vector<snu::KJet> jetColl;
 
  if(jetid.Contains("NONE")) return jetColl;

  eventbase->GetJetSel()->SelectJets(jetColl,jetid, muons, electrons, Option, ptcut, etacut);

  return jetColl;
  
}

std::vector<snu::KJet> AnalyzerCore::GetJetsAwayFromFatJet(TString jetid,   std::vector<snu::KFatJet> fatjetColl,   vector<snu::KMuon>  muons, vector<snu::KElectron>  electrons,  TString Option,float ptcut, float etacut){

  std::vector<snu::KJet> jetColl;

  eventbase->GetJetSel()->SelectJets(jetColl,jetid,  fatjetColl , muons, electrons, Option, ptcut, etacut);
...
std::vector<snu::KFatJet> AnalyzerCore::GetFatJets(TString fatjetid, vector<snu::KMuon>  muons, vector<snu::KElectron>  electrons,   TString Option, float ptcut, float etacut){

  std::vector<snu::KFatJet> fatjetColl;

  eventbase->GetFatJetSel()->SelectFatJets(fatjetColl,fatjetid,  muons, electrons,  Option, ptcut, etacut);
  
  return fatjetColl;

}

std::vector<snu::KFatJet> AnalyzerCore::GetFatJets(TString fatjetid,   TString Option, float ptcut, float etacut){

  std::vector<snu::KFatJet> fatjetColl;
  vector<snu::KMuon>  muons; vector<snu::KElectron>  electrons;
  eventbase->GetFatJetSel()->SelectFatJets(fatjetColl,fatjetid,  muons, electrons,  Option, ptcut, etacut);
...
std::vector<snu::KMuon> AnalyzerCore::GetMuons(TString muid, TString Option,  float ptcut, float etacut){
  return GetMuons(muid, true, Option, ptcut, etacut);
}

std::vector<snu::KMuon> AnalyzerCore::GetMuons(TString muid, bool keepfakes, TString Option, float ptcut, float etacut){

  std::vector<snu::KMuon> muonColl;
  
  if(muid.Contains("NONE")) return muonColl;

  eventbase->GetMuonSel()->SelectMuons(muonColl,muid, Option, ptcut, etacut);
...
std::vector<snu::KElectron> AnalyzerCore::GetElectrons(TString elid, TString Option,float ptcut, float etacut){

  /// if cf flag set and MC keep CF and conversion electrons
  if(k_running_chargeflip)  return GetElectrons( true, false, elid,Option, ptcut, etacut);
...
std::vector<snu::KElectron> AnalyzerCore::GetElectrons(bool keepcf, bool keepfake,  TString elid, TString Option,float ptcut, float etacut){

  std::vector<snu::KElectron> electronColl;
  
  /// returns an empty vector
  if(elid.Contains("NONE")) return electronColl;
  eventbase->GetElectronSel()->SelectElectrons(electronColl,elid, Option, ptcut, etacut);
...
snu::KTruth AnalyzerCore::GetTruthMatchedParticle(snu::KElectron el){
  ...
  if(UInt_t(el.MCTruthIndex()) >  eventbase->GetTruth().size()){
    snu::KTruth tr;
    return tr;
  }
...
vector<int> AnalyzerCore::GetVirtualMassIndex(int mode, int pdgid){
  ...
  for(unsigned int ig=0; ig < eventbase->GetTruth().size(); ig++){
    ...
    if(fabs(eventbase->GetTruth().at(ig).PdgId()) == pdgid){
      unsigned int index_m=eventbase->GetTruth().at(ig).IndexMother() ;
      if(eventbase->GetTruth().at(ig).GenStatus() ==1  || eventbase->GetTruth().at(ig).GenStatus() ==23 ){
	unsigned int daughter=ig;
	...
	if(eventbase->GetTruth().at(index_m).PdgId() == 23 || fabs(eventbase->GetTruth().at(index_m).PdgId()) < 6 ){
	  ...
	  for(unsigned int ig2=0; ig2 < eventbase->GetTruth().size(); ig2++){
	    ...
	    if(fabs(eventbase->GetTruth().at(ig2).PdgId()) == pdgid){
	      if(UInt_t(eventbase->GetTruth().at(ig2).IndexMother())==index_m)           indexZ.push_back(ig2);
	    }
	  }
	}
      }
  ...
  if(eventbase->GetTruth().at(ig).IndexMother() <= 0)continue;
    ...
    if(fabs(eventbase->GetTruth().at(ig).PdgId()) == pdgid){
      if(UInt_t(indexZ[0]) == ig || UInt_t(indexZ[1]) == ig ) continue;
	...
	for(unsigned int ig2=0; ig2 < eventbase->GetTruth().size(); ig2++){
	  ...
	  if(fabs(eventbase->GetTruth().at(ig2).PdgId()) == pdgid){
	    if(UInt_t(indexZ[0]) == ig2 || UInt_t(indexZ[1]) == ig2 ) continue;
	    if(ig==ig2) continue;
...
std::vector<snu::KMuon> AnalyzerCore::GetHNTriMuonsByLooseRelIso(double LooseRelIsoMax, bool keepfake){
  ...
  std::vector<snu::KMuon> muontriLooseColl_raw = GetMuons("MUON_HN_TRI_VLOOSE", keepfake, "roch");
...
bool AnalyzerCore::PassID(snu::KMuon mu, TString muid){
  ...
  return eventbase->GetMuonSel()->MuonPass(mu,muid); 
...
bool AnalyzerCore::PassID(snu::KElectron el, TString elid){
  ...
  return eventbase->GetElectronSel()->ElectronPass(el, elid);
