// $Id: Apr2017_ZGto4lTest_ee.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQApr2017_ZGto4lTest_ee Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
 #include "Apr2017_ZGto4lTest_ee.h"
 ...
 ClassImp (Apr2017_ZGto4lTest_ee);

 Apr2017_ZGto4lTest_ee::Apr2017_ZGto4lTest_ee() : AnalyzerCore(), out_muons(0) {

   SetLogName("Apr2017_ZGto4lTest_ee");
   Message("In Apr2017_ZGto4lTest_ee constructor", INFO);
   InitialiseAnalysis();
 }


 void Apr2017_ZGto4lTest_ee::InitialiseAnalysis() throw( LQError ) {
...
void Apr2017_ZGto4lTest_ee::ExecuteEvents()throw( LQError ){
...
//     if(!k_sample_name.Contains("ZGto2LG")) return;
     ...
     int midxfinal=-1, mpidfinal=0, grmaidxfinal=-1, grmapidfinal=0;
     bool dRMatched=false, dRMatchedOnlydR=false; int Nmatched=0, NmatchedOnlydR=0;
     for(int i=2; i<truthColl.size(); i++){
       ...
       int itmidx=midx, itgrmaidx=-1;
       if(fabs(pid)==11){

         if(electronColl.at(0).DeltaR(truthColl.at(i))>0.1) continue;
         FillHist("DPtReleeT", fabs(electronColl.at(0).Pt()-truthColl.at(i).Pt())/electronColl.at(0).Pt(), weight, 0., 1., 200);
         NmatchedOnlydR++; dRMatchedOnlydR=true;
         if(fabs(electronColl.at(0).Pt()-truthColl.at(i).Pt())/electronColl.at(0).Pt()>0.2) continue;
         dRMatched=true; Nmatched++;
        
         while(truthColl.at(itmidx).PdgId()==pid){
           itmidx=truthColl.at(itmidx).IndexMother();
         }
         midxfinal=itmidx; mpidfinal=truthColl.at(itmidx).PdgId();
         itgrmaidx=truthColl.at(midxfinal).IndexMother();

         if(itgrmaidx!=-1){
           while(truthColl.at(itgrmaidx).PdgId()==mpidfinal){
             itgrmaidx=truthColl.at(itgrmaidx).IndexMother();
           }
           grmaidxfinal=itgrmaidx; grmapidfinal=truthColl.at(itgrmaidx).PdgId();
         }
       }
       else if(pid==22){
         if(electronColl.at(0).DeltaR(truthColl.at(i))>0.3) continue;
         FillHist("dReg", electronColl.at(0).DeltaR(truthColl.at(i)), weight, 0., 5., 500);
         if(truthColl.at(i).Pt()>0){
           FillHist("PtReleg", electronColl.at(0).Pt()/truthColl.at(i).Pt(), weight, 0., 2., 200);
           FillHist("PtRelge", truthColl.at(i).Pt()/electronColl.at(0).Pt(), weight, 0., 2., 200);
         }
         if(fabs(truthColl.at(i).Pt()-electronColl.at(0).Pt())/electronColl.at(0).Pt()>0.2) continue;
         dRMatched=true; Nmatched++;
     ...
     FillHist("Nmatched", Nmatched, weight, 0., 5., 5);

     FillHist("PhotonComposition", 0., weight, 0., 10., 10);//TotalEvtPassing
     if(dRMatched) FillHist("PhotonComposition", 1., weight, 0., 10., 10);//TotalEleMatchedEvt
     if(mpidfinal!=0){
       if(mpidfinal==22)       FillHist("PhotonComposition", 2., weight, 0., 10., 10);//ExplicitlyPhotonDaughterEvt
       if(grmaidxfinal!=-1){
         if(mpidfinal==22 && fabs(grmapidfinal)>40) FillHist("PhotonComposition", 3., weight, 0., 10., 10);//PhotonConvButFake
       }
       if(fabs(mpidfinal)>40)  FillHist("PhotonComposition", 4., weight, 0., 10., 10);//HadronDecayFakeEle
       if(fabs(mpidfinal)==15) FillHist("PhotonComposition", 5., weight, 0., 10., 10);//TauDecayFakeEle
     }
     if(!dRMatched) FillHist("PhotonComposition", 6., weight, 0., 10., 10);
   ...
   if(TriMu_analysis){
     ...
     for(int i=2; i<truthColl.size(); i++){
       ...
       if(fabs(pid)==13){
         ...
         if(electronColl.at(0).DeltaR(truthColl.at(i))>0.1) return;
         FillHist("DPtReleeT", fabs(electronColl.at(0).Pt()-truthColl.at(i).Pt())/electronColl.at(0).Pt(), weight, 0., 1., 200);
//         if(fabs(electronColl.at(0).Pt()-truthColl.at(i).Pt())/electronColl.at(0).Pt()>0.2) return;
         dRMatched=true;
...
void Apr2017_ZGto4lTest_ee::EndCycle()throw( LQError ){
...
void Apr2017_ZGto4lTest_ee::BeginCycle() throw( LQError ){
...
Apr2017_ZGto4lTest_ee::~Apr2017_ZGto4lTest_ee() {
  
  Message("In Apr2017_ZGto4lTest_ee Destructor" , INFO);
//  if(!k_isdata)delete reweightPU;
  
}

void Apr2017_ZGto4lTest_ee::FillCutFlow(TString cut, float weight){
...
void Apr2017_ZGto4lTest_ee::BeginEvent( )throw( LQError ){
...
void Apr2017_ZGto4lTest_ee::MakeHistograms(){
  ...
  AnalyzerCore::MakeHistograms("PhotonComposition", 10, 0., 10.);
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(1,"Total");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(2,"Matched");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(3,"#gamma>ee");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(4,"had>#gamma>ee");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(5,"had>e");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(6,"#tau>e");
    GetHist("PhotonComposition")->GetXaxis()->SetBinLabel(7,"UnMatched");

  Message("Made histograms", INFO);

  // **
  // *  Remove//Overide this Apr2017_ZGto4lTest_eeCore::MakeHistograms() to make new hists for your analysis
  // **
...
void Apr2017_ZGto4lTest_ee::ClearOutputVectors() throw(LQError) {
