// $Id: Jun2017_GenMatchXCheck.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQJun2017_GenMatchXCheck Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
 #include "Jun2017_GenMatchXCheck.h"
 ...
 ClassImp (Jun2017_GenMatchXCheck);

 Jun2017_GenMatchXCheck::Jun2017_GenMatchXCheck() : AnalyzerCore(), out_muons(0) {

   SetLogName("Jun2017_GenMatchXCheck");
   Message("In Jun2017_GenMatchXCheck constructor", INFO);
   InitialiseAnalysis();
 }


 void Jun2017_GenMatchXCheck::InitialiseAnalysis() throw( LQError ) {
...
void Jun2017_GenMatchXCheck::ExecuteEvents()throw( LQError ){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /// Apply the gen weight 
   ...
   bool EleTypeXCheck=false, QCDAllLepUseCheck=false;
   for(int i=0; i<k_flags.size(); i++){
     if(k_flags.at(i).Contains("EleTypeXCheck")) EleTypeXCheck=true;
     else if(k_flags.at(i).Contains("QCDAllLepUseCheck")) QCDAllLepUseCheck=true;
   }
...
/************************************************************************************/
   ...
   if(EleTypeXCheck){
     ...
     std::vector<snu::KElectron> electron40Coll;
       for(int i=0; i<electronPreColl.size(); i++){ if(electronPreColl.at(i).GetType()==40) electron40Coll.push_back(electronPreColl.at(i)); }
     
     FillHist("NEle40", electron40Coll.size(), weight, 0., 10., 10);
     if(electron40Coll.size()!=0){
       //PrintTruth();
       for(int i=0; i<electron40Coll.size(); i++){
         int MatchedIdx     = GenMatchedIdx(electron40Coll.at(i), truthColl);
         int LeptonType     = GetLeptonType(electron40Coll.at(i), truthColl);
         int NearPhotonIdx  = GetNearPhotonIdx(electron40Coll.at(i), truthColl);
         int NearPhotonIdxTemp  = GetNearPhotonIdxTemp(electron40Coll.at(i), truthColl);
         int NearPhotonType = GetPhotonType(NearPhotonIdx, truthColl);
         FillHist("Ele40LepType", LeptonType, weight, -10., 10., 20);

         if(LeptonType==1 && NearPhotonIdxTemp!=-1){
           FillHist("Ele40Lep1dReg", electron40Coll.at(i).DeltaR(truthColl.at(NearPhotonIdxTemp)), weight, 0., 0.4, 400);
           FillHist("Ele40Lep1RelPtge", truthColl.at(NearPhotonIdxTemp).Pt()/electron40Coll.at(i).Pt(), weight, 0., 10., 1000);
         }
         //cout<<"MatchedIdx: "<<MatchedIdx<<" LeptonType: "<<LeptonType<<" NearPhotonIdx: "<<NearPhotonIdx<<" NearPhotonType: "<<NearPhotonType<<endl;
         //cout<<"ElePt: "<<electron40Coll.at(i).Pt()<<" EleEta: "<<electron40Coll.at(i).Eta()<<" ElePhi: "<<electron40Coll.at(i).Phi()<<endl;
       }
     }
     
   }//End of Ele Fake WP Scan
   if(QCDAllLepUseCheck){

      for(int i=0; i<electronPreColl.size(); i++){
        int LepType=GetLeptonType(electronPreColl.at(i), truthColl);
        FillHist("LepType", LepType, weight, -10., 10., 20);
      }
...
void Jun2017_GenMatchXCheck::EndCycle()throw( LQError ){
...
void Jun2017_GenMatchXCheck::BeginCycle() throw( LQError ){
...
Jun2017_GenMatchXCheck::~Jun2017_GenMatchXCheck() {
  
  Message("In Jun2017_GenMatchXCheck Destructor" , INFO);
//  if(!k_isdata)delete reweightPU;
  
}

int Jun2017_GenMatchXCheck::GetNearPhotonIdxTemp(snu::KElectron Ele, std::vector<snu::KTruth>& TruthColl, TString Option){//1)
...
float Jun2017_GenMatchXCheck::FakeRateMC(snu::KElectron Ele, TString Option){
...
int Jun2017_GenMatchXCheck::GetFakeLepSrcType(snu::KElectron Ele, std::vector<snu::KJet> JetColl){
...
float Jun2017_GenMatchXCheck::ConeCorrectedPT(snu::KElectron Ele, float TightIsoCut){
...
int Jun2017_GenMatchXCheck::StepPassed(std::vector<snu::KMuon> MuColl, std::vector<snu::KElectron> EleColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, TString Option){
...
int Jun2017_GenMatchXCheck::NPromptFake_Mu(std::vector<snu::KMuon> MuColl, std::vector<snu::KTruth> truthColl, TString Option){
...
int Jun2017_GenMatchXCheck::NPromptFake_Ele(std::vector<snu::KElectron> EleColl, std::vector<snu::KTruth> truthColl, TString Option){
...
bool Jun2017_GenMatchXCheck::IsConvCand(snu::KElectron Ele, std::vector<snu::KTruth> TruthColl, TString Option){
...
void Jun2017_GenMatchXCheck::FillCutFlow(TString cut, float weight){
...
void Jun2017_GenMatchXCheck::BeginEvent( )throw( LQError ){
...
void Jun2017_GenMatchXCheck::MakeHistograms(){
  ...
  // **
  // *  Remove//Overide this Jun2017_GenMatchXCheckCore::MakeHistograms() to make new hists for your analysis
  // **
...
void Jun2017_GenMatchXCheck::ClearOutputVectors() throw(LQError) {
