// $Id: Mar2017_WZControl.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQMar2017_WZControl Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
 ...
 ClassImp (Mar2017_WZControl);

 Mar2017_WZControl::Mar2017_WZControl() : AnalyzerCore(), out_muons(0) {

   SetLogName("Mar2017_WZControl");
   Message("In Mar2017_WZControl constructor", INFO);
   InitialiseAnalysis();
 }


 void Mar2017_WZControl::InitialiseAnalysis() throw( LQError ) {
...
void Mar2017_WZControl::ExecuteEvents()throw( LQError ){
...
/*
     bool TrigZZ=false;
   G  for(int i=0; i<muonVetoColl.size(); i++){
       if( muonVetoColl.at(i).IsTight() && muonVetoColl.at(i).dXY()<0.05 && muonVetoColl.at(i).dXYSig()<3. 
          && (muonVetoColl.at(i).RelIso04()*muonVetoColl.at(i).Pt()/muonVetoColl.at(i).RochPt())<0.4 
          && muonVetoColl.at(i).Pt()>10. ) continue;

       if(fabs((muonVetoColl.at(i)+muonColl.at(IdxWCand)).M()-91.2)<10.) TrigZZ=true;
       if(fabs((muonColl.at(0)+muonColl.at(1)+muonColl.at(2)+muonVetoColl.at(i)).M()-91.2)<10.) TrigZZ=true;
     }
     if(!TrigZZ) FillHist("TrigZZ", 0., weight, 0., 2., 2);
     else        FillHist("TrigZZ", 1., weight, 0., 2., 2);

     if(TrigZZ) return;
     FillCutFlow("ZZVeto", weight);
*/


     if(muonColl.at(2).Pt()<15) return;
     FillCutFlow("PT#mu15", weight);

     FillHist("MET_3lM4MZPT15Cut_3mu", met, weight, 0., 200., 200);
     FillHist("MTW_3lM4MZPT15Cut_3mu", MTW, weight, 0., 300., 300); 


     if(MTW<60) return;
     FillCutFlow("MTW60", weight);
 
     FillHist("Nj_3lM4MZPT15MTW60Cut_3mu", njets, weight, 0., 10., 10);
 

     if(nbjets!=0) return;
     FillCutFlow("BVeto", weight);

     FillHist("Nj_3lM4MZPT15MTW60NobCut_3mu", njets, weight, 0., 10., 10);
     FillHist("MTW_3lM4MZPT15MTW60NobCut_3mu", MTW, weight, 0., 300., 300); 
...
void Mar2017_WZControl::EndCycle()throw( LQError ){
...
void Mar2017_WZControl::BeginCycle() throw( LQError ){
...
Mar2017_WZControl::~Mar2017_WZControl() {
  
  Message("In Mar2017_WZControl Destructor" , INFO);
//  if(!k_isdata)delete reweightPU;
  
}

void Mar2017_WZControl::FillCutFlow(TString cut, float weight){
  ...
  if(GetHist("cutflow_W") && GetHist("cutflow_N")){
    ...
    if(!GetHist("cutflow_W")){
      ...
      GetHist("cutflow_W")->GetXaxis()->SetBinLabel(7,"MZCut");
      GetHist("cutflow_W")->GetXaxis()->SetBinLabel(8,"PT#mu15");
      GetHist("cutflow_W")->GetXaxis()->SetBinLabel(9,"MTW60");
      GetHist("cutflow_W")->GetXaxis()->SetBinLabel(10,"BVeto");
    }
    if(!GetHist("cutflow_N")){
      ...
      GetHist("cutflow_N")->GetXaxis()->SetBinLabel(7,"MZCut");
      GetHist("cutflow_N")->GetXaxis()->SetBinLabel(8,"PT#mu15");
      GetHist("cutflow_N")->GetXaxis()->SetBinLabel(9,"MTW60");
      GetHist("cutflow_N")->GetXaxis()->SetBinLabel(10,"BVeto");
    }
  }
}
...
void Mar2017_WZControl::BeginEvent( )throw( LQError ){
...
void Mar2017_WZControl::MakeHistograms(){
  ...
  // **
  // *  Remove//Overide this Mar2017_WZControlCore::MakeHistograms() to make new hists for your analysis
  // **
...
void Mar2017_WZControl::ClearOutputVectors() throw(LQError) {
