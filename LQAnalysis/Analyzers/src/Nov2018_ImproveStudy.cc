// $Id: Nov2018_ImproveStudy.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQNov2018_ImproveStudy Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
 #include "Nov2018_ImproveStudy.h"
 ...
 ClassImp (Nov2018_ImproveStudy);

 Nov2018_ImproveStudy::Nov2018_ImproveStudy() : AnalyzerCore(), out_muons(0) {

   SetLogName("Nov2018_ImproveStudy");
   Message("In Nov2018_ImproveStudy constructor", INFO);
   InitialiseAnalysis();
 }


 void Nov2018_ImproveStudy::InitialiseAnalysis() throw( LQError ) {
...
void Nov2018_ImproveStudy::ExecuteEvents()throw( LQError ){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /// Apply the gen weight 
   ...
   bool EMuMu=false, TriMu=false, NoTrig=false;
   bool MiniMultiIso=false;
   bool SystRun=false;
   TString Cycle="";
   for(int i=0; i<(int) k_flags.size(); i++){
     if     (k_flags.at(i).Contains("SystRun"))       SystRun       = true;
     else if(k_flags.at(i).Contains("MiniMultiIso")) {MiniMultiIso  = true; Cycle="MiniMultiIso";}
     ...
     else if(k_flags.at(i).Contains("NoTrig"))        NoTrig        = true;
   ...
   ***************************************************************************************/
   ...
   if(NoTrig){ Pass_Trigger=true; trigger_ps_weight=WeightByTrigger("HLT_IsoMu24_v", TargetLumi); }
...
//         trk_weight_mu   = mcdata_correction->MuonTrackingEffScaleFactor(muonColl);//No longer recommendation
   ...
   if(MiniMultiIso){
     ...
     std::vector<snu::KMuon> muonSUSYFakeLColl, muonSUSYLColl, muonSUSYMColl, muonSUSYTColl, muonMiniColl, muonMiniRelColl;
     TVector3 v_jml, v_m, v_j;
     for(int it_m=0; it_m<(int) muonPreColl.size(); it_m++){
       
       int  LepType = GetLeptonType(muonPreColl.at(it_m), truthColl);
       bool IsFake  = LepType>-5 && LepType<0, IsPrompt = LepType>0 && LepType<4;
       bool PassTightID=false, PassTightIP=false, PassMiniIso=false, PassPtRatio=false, PassPtRel=false, NoNearJet=true; 
       bool PassLooseIP=false, PassLooseMiniIso=false;
       float MiniIso = muonPreColl.at(it_m).RelMiniIso();
       float RelIso  = muonPreColl.at(it_m).RelIso04();
       float PtRatio = -1., PtRelv0=-1., PtRelv1=-1.;

       if     ( IsPrompt ) FillHist("MiniIso_Pr", MiniIso, weight, 0., 1., 20);
       else if( IsFake   ) FillHist("MiniIso_Fk", MiniIso, weight, 0., 1., 20);

       if( muonPreColl.at(it_m).IsTight() ) PassTightID = true;
       else continue;
       if( MiniIso<0.4  ) PassLooseMiniIso = true;
       else continue;
       if( fabs(muonPreColl.at(it_m).dXY())<0.1  && fabs(muonPreColl.at(it_m).dZ() )<0.2  && fabs(muonPreColl.at(it_m).dXYSig())<4. 
         ) PassLooseIP = true;
       else continue;

       if( fabs(muonPreColl.at(it_m).dXY())<0.01 && fabs(muonPreColl.at(it_m).dZ() )<0.05 && fabs(muonPreColl.at(it_m).dXYSig())<4. 
         ) PassTightIP = true;
       if( MiniIso<0.14 ) PassMiniIso      = true;
       
       for(int it_j=0; it_j<(int) jetNoVeto10Coll.size(); it_j++){
         if(muonPreColl.at(it_m).DeltaR(jetNoVeto10Coll.at(it_j))>0.4) continue;
         else NoNearJet=false;
         
         float TmpPtRatio = muonPreColl.at(it_m).Pt()/jetNoVeto10Coll.at(it_j).Pt();
         if(TmpPtRatio>PtRatio) PtRatio = TmpPtRatio;

         if(PtRatio>0.68) PassPtRatio=true;

//         if(!PassPtRatio){
         v_jml.SetXYZ(jetNoVeto10Coll.at(it_j).Px()-muonPreColl.at(it_m).Px(),
                      jetNoVeto10Coll.at(it_j).Py()-muonPreColl.at(it_m).Py(),
                      jetNoVeto10Coll.at(it_j).Pz()-muonPreColl.at(it_m).Pz());
         v_j.SetXYZ(jetNoVeto10Coll.at(it_j).Px(),
                    jetNoVeto10Coll.at(it_j).Py(),
                    jetNoVeto10Coll.at(it_j).Pz());
         float Absv_jml = v_jml.Mag();
         float Absv_j   = v_j.Mag();
         v_jml *= Absv_jml!=0? 1./Absv_jml:0.;
         v_j   *= Absv_j  !=0? 1./Absv_j  :0.;
         v_m.SetXYZ(muonPreColl.at(it_m).Px(), muonPreColl.at(it_m).Py(), muonPreColl.at(it_m).Pz());
         float TmpPtRelv0 = v_m.Cross(v_j).Mag();
         float TmpPtRelv1 = v_m.Cross(v_jml).Mag();
         if(TmpPtRelv0>PtRelv0) PtRelv0 = TmpPtRelv0;
         if(TmpPtRelv1>PtRelv1) PtRelv1 = TmpPtRelv1;
         if(PtRelv1>6.7) PassPtRel=true;
//         }
//         if(PassPtRel or PassPtRatio) break;
       }
       if     ( IsPrompt ){
         FillHist("PtRatio_Pr", PtRatio, weight, 0., 1.2, 120);
         FillHist("PtRelv0_Pr", PtRelv0, weight, 0., 20., 40);
         FillHist("PtRelv1_Pr", PtRelv1, weight, 0., 20., 40);
         if(PassMiniIso){
           FillHist("PtRatio_IsoPr", PtRatio, weight, 0., 1.2, 120);
           FillHist("PtRelv0_IsoPr", PtRelv0, weight, 0., 20., 40);
           FillHist("PtRelv1_IsoPr", PtRelv1, weight, 0., 20., 40);
         }
       }
       else if( IsFake   ){
         FillHist("PtRatio_Fk", PtRatio, weight, 0., 1.2, 120);
         FillHist("PtRelv0_Fk", PtRelv0, weight, 0., 20., 40);
         FillHist("PtRelv1_Fk", PtRelv1, weight, 0., 20., 40);
         if(PassMiniIso){
           FillHist("PtRatio_IsoFk", PtRatio, weight, 0., 1.2, 120);
           FillHist("PtRelv0_IsoFk", PtRelv0, weight, 0., 20., 40);
           FillHist("PtRelv1_IsoFk", PtRelv1, weight, 0., 20., 40);
         }
       }

       if(PassTightID and PassTightIP and MiniIso<0.20
         ) muonMiniColl.push_back(muonPreColl.at(it_m));
       if(PassTightID and PassTightIP and MiniIso<0.20 and RelIso<0.4
         ) muonMiniRelColl.push_back(muonPreColl.at(it_m));
       if(PassTightID and PassTightIP and MiniIso<0.22 and (PtRatio>0.63 or PtRelv1>6   or NoNearJet)
         ) muonSUSYLColl.push_back(muonPreColl.at(it_m));
       if(PassTightID and PassTightIP and MiniIso<0.14 and (PtRatio>0.68 or PtRelv1>6.7 or NoNearJet)
         ) muonSUSYMColl.push_back(muonPreColl.at(it_m));
       if(PassTightID and PassTightIP and MiniIso<0.10 and (PtRatio>0.70 or PtRelv1>7   or NoNearJet)
         ) muonSUSYTColl.push_back(muonPreColl.at(it_m));
       if(PassTightID and PassLooseIP and PassLooseMiniIso
         ) muonSUSYFakeLColl.push_back(muonPreColl.at(it_m));
     }
     std::vector<snu::KJet> jetSUSYColl  = SkimJetColl(jetNoVeto10Coll,  electronLooseColl, muonSUSYFakeLColl, "EleMuVeto");
     std::vector<snu::KJet> bjetSUSYColl = SelBJets(jetSUSYColl, "Medium");

     if(EMuMu){
       CheckMiniMultiIso(muonColl, muonLooseColl, electronColl, electronLooseColl, jetColl, bjetColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "", "EMuMu");
       CheckMiniMultiIso(muonMiniRelColl, muonLooseColl, electronColl, electronLooseColl, jetColl, bjetColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "_MiniRel", "EMuMu");
       CheckMiniMultiIso(muonMiniColl, muonSUSYFakeLColl, electronColl, electronLooseColl, jetSUSYColl, bjetSUSYColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "_Mini", "EMuMu");
       CheckMiniMultiIso(muonSUSYLColl, muonSUSYFakeLColl, electronColl, electronLooseColl, jetSUSYColl, bjetSUSYColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "_SUSYL", "EMuMu");
       CheckMiniMultiIso(muonSUSYMColl, muonSUSYFakeLColl, electronColl, electronLooseColl, jetSUSYColl, bjetSUSYColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "_SUSYM", "EMuMu");
       CheckMiniMultiIso(muonSUSYTColl, muonSUSYFakeLColl, electronColl, electronLooseColl, jetSUSYColl, bjetSUSYColl, met, met*cos(metphi), met*sin(metphi),
                         weight, "_SUSYT", "EMuMu");
     }
     if(TriMu){
       CheckMiniMultiIso(muonColl, muonLooseColl, electronColl, electronLooseColl, jetColl, bjetColl, met, met*cos(metphi), met*sin(metphi),
...
void Nov2018_ImproveStudy::EndCycle()throw( LQError ){
...
void Nov2018_ImproveStudy::BeginCycle() throw( LQError ){
...
Nov2018_ImproveStudy::~Nov2018_ImproveStudy() {
  
  Message("In Nov2018_ImproveStudy Destructor" , INFO);
//  if(!k_isdata)delete reweightPU;
  
}


void Nov2018_ImproveStudy::CheckMiniMultiIso(std::vector<snu::KMuon> MuTColl, std::vector<snu::KMuon> MuLColl, std::vector<snu::KElectron> EleTColl, std::vector<snu::KElectron> EleLColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, float METx, float METy, float weight, TString Label, TString Option){
  ...
  //if(!isData && (k_sample_name.Contains("TT_powheg") || k_sample_name.Contains("TTLL_powheg"))) weight*=1.5;
  ...
  if(EMuMu){
    ...
    if(!isData && (k_sample_name.Contains("TT_powheg") || k_sample_name.Contains("TTLL_powheg"))){
      std::vector<snu::KTruth> truthColl; eventbase->GetTruthSel()->Selection(truthColl);
      int NFk=0;
      for(int it_lep=0; it_lep<(int) EleTColl.size(); it_lep++){
        int LepType = GetLeptonType(EleTColl.at(it_lep),truthColl);
        if(LepType<0 && LepType>-5) NFk++;
      }
      for(int it_lep=0; it_lep<(int) MuTColl.size(); it_lep++){
    ...
    if(k_sample_name.Contains("TTToHcToWA")) FillHist("NSigBkg"+Label, 0., weight, 0., 2., 2);
    else                                     FillHist("NSigBkg"+Label, 1., weight, 0., 2., 2);
  }
  if(TriMu){
...
void Nov2018_ImproveStudy::CheckSRDist(std::vector<snu::KMuon> MuTColl, std::vector<snu::KMuon> MuLColl, std::vector<snu::KElectron> EleTColl, std::vector<snu::KElectron> EleLColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, float METx, float METy, float weight, TString Label, TString Option){
  ...
  if(EMuMu){
    ...
    if(fabs(Mmumu-91.2)<10) return;
      ...
      for(int it_cent=0; it_cent<(int) Mmumu_CentFull.size(); it_cent++){
        if(Label!="") continue;
        float LargeWidth=5.;
...
void Nov2018_ImproveStudy::CheckLowMass(std::vector<snu::KMuon> MuTColl, std::vector<snu::KMuon> MuLColl, std::vector<snu::KElectron> EleTColl, std::vector<snu::KElectron> EleLColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, float METx, float METy, float weight, TString Label, TString Option){
...
void Nov2018_ImproveStudy::DoSystRun(TString Cycle, TString Mode, std::vector<snu::KElectron> EleColl, std::vector<snu::KElectron> EleLColl, std::vector<snu::KMuon> MuColl, std::vector<snu::KMuon> MuLColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, float METx, float METy, float weight){
...
float Nov2018_ImproveStudy::ConeCorrectedPT(snu::KMuon Mu, float TightIsoCut){
...
float Nov2018_ImproveStudy::ConeCorrectedPT(snu::KElectron Ele, float TightIsoCut){
...
void Nov2018_ImproveStudy::FillCutFlow(TString cut, float weight){
...
void Nov2018_ImproveStudy::BeginEvent( )throw( LQError ){
...
void Nov2018_ImproveStudy::MakeHistograms(){
  ...
  // **
  // *  Remove//Overide this Nov2018_ImproveStudyCore::MakeHistograms() to make new hists for your analysis
  // **
...
void Nov2018_ImproveStudy::ClearOutputVectors() throw(LQError) {
