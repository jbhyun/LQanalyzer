// $Id: Jan2018_EleIPQuickCheck.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQJan2018_EleIPQuickCheck Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
 #include "Jan2018_EleIPQuickCheck.h"
 ...
 ClassImp (Jan2018_EleIPQuickCheck);

 Jan2018_EleIPQuickCheck::Jan2018_EleIPQuickCheck() : AnalyzerCore(), out_muons(0) {

   SetLogName("Jan2018_EleIPQuickCheck");
   Message("In Jan2018_EleIPQuickCheck constructor", INFO);
   InitialiseAnalysis();
 }


 void Jan2018_EleIPQuickCheck::InitialiseAnalysis() throw( LQError ) {
...
void Jan2018_EleIPQuickCheck::ExecuteEvents()throw( LQError ){
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  /// Apply the gen weight 
   ...
   bool SystRun=false;
   bool EleIPCheck=false;
   for(int i=0; i<k_flags.size(); i++){
     if     (k_flags.at(i).Contains("EleIPCheck"))  EleIPCheck = true;
   ...
   ***************************************************************************************/
   ...
   std::vector<snu::KElectron> electronPreColl; eventbase->GetElectronSel()->Selection(electronPreColl);
    if(EleIPCheck){ if(!electronPreColl.size()>0) return;}
   //******************************************************************************************************//
...
//
     ...
     eventbase->GetElectronSel()->SetBETrRegIncl(false);
     eventbase->GetElectronSel()->SetApplyConvVeto(true);
   std::vector<snu::KElectron> electronNoIsoIPColl; eventbase->GetElectronSel()->Selection(electronNoIsoIPColl);
...
//         fake_weight=GetFakeWeight(muonLooseColl, electronLooseColl, "HNTrilepFakeL2", "HNTrilepTight2", "HctoWAFakeLoose","POGMVAMIP");
   ...
   if(EleIPCheck){
     ...
     CheckIPIsoComp(electronNoIsoIPColl, jetColl, bjetColl, truthColl, met, met*cos(metphi), met*sin(metphi), weight, "", "");
...
void Jan2018_EleIPQuickCheck::EndCycle()throw( LQError ){
...
void Jan2018_EleIPQuickCheck::BeginCycle() throw( LQError ){
...
Jan2018_EleIPQuickCheck::~Jan2018_EleIPQuickCheck() {
  
  Message("In Jan2018_EleIPQuickCheck Destructor" , INFO);
...
void Jan2018_EleIPQuickCheck::CheckIPIsoComp(std::vector<snu::KElectron> EleColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, std::vector<snu::KTruth> TruthColl, float MET, float METx, float METy, float weight, TString Label, TString Option){
  ...
  int LepType=GetLeptonType(EleColl.at(it_el), TruthColl);
    ...
    if(LepType>0){
      ...
      FillHist("fdxy_Pr", fabs(EleColl.at(it_el).dxy()), weight, 0., 1.,  200);
      FillHist("fdz_Pr", fabs(EleColl.at(it_el).dz()), weight, 0., 1.,  200);
      FillHist("fdxysig_Pr", fabs(EleColl.at(it_el).dxySig()), weight, 0., 10.,  100);
    }
    if(LepType<0){
      FillHist("RelIso_Fk", EleColl.at(it_el).PFRelIso(0.3), weight, 0., 2., 200);
      FillHist("fdxy_Fk", fabs(EleColl.at(it_el).dxy()), weight, 0., 1.,  200);
      FillHist("fdz_Fk", fabs(EleColl.at(it_el).dxy()), weight, 0., 1.,  200);
      FillHist("fdxysig_Fk", fabs(EleColl.at(it_el).dxySig()), weight, 0., 10.,  100);
    }
  }
...
void Jan2018_EleIPQuickCheck::DoSystRun(TString Cycle, TString Mode, std::vector<snu::KElectron> EleColl, std::vector<snu::KElectron> EleLColl, std::vector<snu::KMuon> MuColl, std::vector<snu::KMuon> MuLColl, std::vector<snu::KJet> JetColl, std::vector<snu::KJet> BJetColl, float MET, float METx, float METy, float weight){
...
float Jan2018_EleIPQuickCheck::ConeCorrectedPT(snu::KMuon Mu, float TightIsoCut){
...
float Jan2018_EleIPQuickCheck::ConeCorrectedPT(snu::KElectron Ele, float TightIsoCut){
...
float Jan2018_EleIPQuickCheck::FakeRateData(snu::KMuon Mu, TString Option){
...
float Jan2018_EleIPQuickCheck::FakeRateData(snu::KElectron Ele, TString Option){
...
//float Jan2018_EleIPQuickCheck::OptimiseSelection(std::vector<snu::KMuon> MuPreColl, std::vector<snu::KElectron> ElePreColl, std::vector<snu::KJet> JetPreNoVetoColl, TString MuLID, TString MuTID, TString EleLID, TString EleTID, TString Label){
//
//  float Mmumu=
//  if(
//
...
float Jan2018_EleIPQuickCheck::GetFakeWeight(std::vector<snu::KMuon> MuLColl, std::vector<snu::KElectron> EleLColl, TString MuLID, TString MuTID, TString EleLID, TString EleTID, TString Option){
...
float Jan2018_EleIPQuickCheck::GetPreTrigPURW(int Nvtx){
...
void Jan2018_EleIPQuickCheck::FillCutFlow(TString cut, float weight){
...
void Jan2018_EleIPQuickCheck::BeginEvent( )throw( LQError ){
...
void Jan2018_EleIPQuickCheck::MakeHistograms(){
  ...
  // **
  // *  Remove//Overide this Jan2018_EleIPQuickCheckCore::MakeHistograms() to make new hists for your analysis
  // **
...
void Jan2018_EleIPQuickCheck::ClearOutputVectors() throw(LQError) {
