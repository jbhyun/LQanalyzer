...
namespace snu {
  ...
  public:
    ...
    // New functions for custom ids
    Double_t GetIDIso(TString IDlabel, TString isotype);
    bool PassCustomID(TString IDlabel);
    ...
    inline Double_t IsoValue(TString isotype){
      if ( isotype == "reliso_03") return PFRelIso(0.3);
      return -999.;
    }
    
    Bool_t ElInCrackRegion() const {
      if( fabs(k_sceta)>1.4442 && fabs(k_sceta)<1.566 ) return true;
      else return false; 
    }
