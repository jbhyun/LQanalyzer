...
bool KMuon::PassCustomID(TString IDlabel){
  ...
  // -- ID HNPair Loose
  else if (CheckIDList(IDlabel ,"MUON_HN_NN_HighPt",IDlist)){
    
    if(!this->PassCustomID("MUON_PTETA"))  pass=false;
    if(!this->IsHighPt())  pass=false;
    return pass;
  }
...
Double_t KMuon::GetIDIso(TString IDlabel, TString isotype){
