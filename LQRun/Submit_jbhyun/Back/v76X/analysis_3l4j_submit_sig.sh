...
## RUN PARAMETERS
AnalysisCode="Oct2016_3l4j_Cutflow" ##"Nov2016_3l4j_4lComposition" ###_D3lv" ###"Jun2016_3l4j_SigBasic" ###KPS ###"ExampleAnalyzerDiMuon"
...
if [[ $NEvent == 'ALL' ]]; then NEvent="";
else echo "Error: Wrong NEvent"; exit 1;
fi 

########################################################################
if [[ $runData == 'false' || $runData == 'False' ]];
  then bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -a ${AnalysisCode} -p ${DataPeriod} -s ${Skim} -list ${MCList} -n ${Njob} -o ${OutputDir} -d ${LogLevel} -events ${NEvent} &> SKTree_${AnalysisCode}_${Stream}_log.txt & 
