...
## RUN PARAMETERS
AnalysisCode="Sep2016_3l4j_PzvStudy" ###_D3lv" ###"Jun2016_3l4j_SigBasic" ###KPS ###"ExampleAnalyzerDiMuon"
Stream="MuonEG" ###"SingleElectron" ### DoubleMuon DoubleEG MuonEG SingleElectron SingleMuon 
Skim="SKTree_NoSkim" ###"Lepton" ### "Lepton"(single lepton skim)/"DiLep"(dilepton skim)/"NoCut"(noskim) ### SKTree_NoSkim/SKTree_LeptonSkim/SKTree_Di[Tri]LepSkim/ flatcat
DataPeriod="CtoD"   ###  "C" = period C only   "ALL" or "CtoD"  = period C+D
job_logstep=1000
LogLevel="INFO"
Njob=50

MCList="Analysis_sig_1e2mu"
#MCList="Analysis_sig_3mu"
...
if [[ $runData == 'false' || $runData == 'False' ]];
  then bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -a ${AnalysisCode} -p ${DataPeriod} -s ${Skim} -list ${MCList} -n ${Njob} -o ${OutputDir} -d ${LogLevel} 
elif [[ $runMC == 'false' || $runMC == 'False' ]];
  then bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -a ${AnalysisCode} -S ${Stream} -p ${DataPeriod} -s ${Skim} -n ${Njob} -o ${OutputDir} -d ${LogLevel}
else
  bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -a ${AnalysisCode} -S ${Stream} -p ${DataPeriod} -s ${Skim} -list ${MCList} -n ${Njob} -o ${OutputDir} -d ${LogLevel}
fi
########################################################################
