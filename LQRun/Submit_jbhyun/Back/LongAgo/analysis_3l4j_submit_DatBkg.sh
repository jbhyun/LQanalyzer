...
## RUN PARAMETERS
job_cycle="Jul2016_3l4j_DataMCComp" ###"Jun2016_3l4j_SigBasic" ###KPS ###"ExampleAnalyzerDiMuon"
job_data_lumi="C"   ###  "C" = period C only   "ALL" or "CtoD"  = period C+D
job_stream="DoubleMuon" ###"SingleElectron" ### DoubleMuon DoubleEG MuonEG SingleElectron SingleMuon 
job_skinput="True" 

job_useskim="DiLep" ###"Lepton" ### "Lepton"(single lepton skim)/"DiLep"(dilepton skim)/"NoCut"(noskim)
job_logstep=1000
job_loglevel="DEBUG"
job_njobs=40
...
if [[ ${job_stream} == 'DoubleMuon' || ${job_stream} == 'SingleMuon' ]]; then outputdir_lep=${outputdir_cat}"/output/Muon/";
elif [[ ${job_stream} == 'DoubleEG' || ${job_stream} == 'SingleElectron' ]]; then outputdir_lep=${outputdir_cat}"/output/Electron/";
elif [[ ${job_stream} == 'MuonEG' ]]; then outputdir_lep=${outputdir_cat}"/output/ElectronMuon/";
...
if [[ ! -d "${outputdir_mc}" ]]; then mkdir ${outputdir_mc}; echo "Made ${outputdir_mc}"; fi
if [[ ! -d "${outputdir_data}" ]]; then mkdir ${outputdir_data}; echo "Making ${outputdir_data}";fi
 

if [[ $1  == "MC"  ]];   then runMC=true;  runData=false; fi
if [[ $1  == "DATA"  ]]; then runMC=false; runData=true;  fi
...
###    declare -a input_samples=( 'DY10to50_MCatNLO' 'DY50plus_MCatNLO' 'WJets_MCatNLO' 'TT_MG5' 'TT_powheg' 'TTG_MCatNLO' 'WZ_lllnu_MCatNLO' 'WZ_llqq_MCatNLO' 'ZZ_llqq_MCatNLO' 'ZZ_llll_MCatNLO' 'singletop_tbarW_Powheg' 'singletop_tW_Powheg' 'singletop_tbar_Powheg' 'singletop_t_Powheg' 'singletop_s_MCatNLO' 'ttHtobb_Powheg' 'ttHnobb_Powheg' 'ttWJetsToLNu_MCatNLO' 'ttZToQQ_MCatNLO' 'ttZToLLNuNu_MCatNLO' )
    declare -a input_samples=( 'TT_MG5' )

###Signal
###    declare -a input_samples=("tthwA_3l4j_hc90A5_3mu" "tthwA_3l4j_hc90A5_emu" "tthwA_3l4j_hc130A5_3mu" "tthwA_3l4j_hc130A5_emu" "tthwA_3l4j_hc130A30_3mu" "tthwA_3l4j_hc130A30_emu" "tthwA_3l4j_hc155A30_3mu" "tthwA_3l4j_hc155A30_emu")
###    declare -a input_samples=("tthwA_3l4j_hc90A5_emu" "tthwA_3l4j_hc130A5_emu" "tthwA_3l4j_hc130A30_emu" "tthwA_3l4j_hc155A30_emu")
###    declare -a input_samples=("tthwA_3l4j_hc130A5_3mu" "tthwA_3l4j_hc130A5_emu")
###    declare -a input_samples=("tthwA_3l4j_hc130A30_emu")
