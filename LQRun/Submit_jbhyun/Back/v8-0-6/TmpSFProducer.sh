...
AnalysisCode="May2017_TmpIDTrigSF"
Stream="MuonEG"
...
QueueOption="fastq" 
#RunningMode="EleIDSF"
#RunningMode="EMuTrigSF,EleLeg"
#RunningMode="EMuTrigSF,MuonLeg"
RunningMode="EMuTrigSF,DZeff"
#RunningMode="EMuTrigSF,MCClosure"
#RunningMode="EMuTrigSF,MCClosure_3l"
#RunningMode="EMuTrigSF,CorrelationStudy,EleEffbyMu"
#RunningMode="EMuTrigSF,CorrelationStudy,MuEffbyEle"
#RunningMode="EMuTrigSF,CorrelationStudy,MuEffbyMu"
...
if [[ $runData == 'false' || $runData == 'False' ]];
  ...
  then nohup bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -b True -a ${AnalysisCode} -S ${Stream} -p ${DataPeriod} -s ${Skim} -o ${OutputDir} -d ${LogLevel} -q ${QueueOption} -userflag ${RunningMode} -fake ${runFake}
  echo "nohup bash ${LQANALYZER_BIN_PATH}/submitSKTree.sh -b True -a ${AnalysisCode} -S ${Stream} -p ${DataPeriod} -s ${Skim} -o ${OutputDir} -d ${LogLevel} -q ${QueueOption} -userflag ${RunningMode} -fake ${runFake}" >> CommandHist.txt
else
